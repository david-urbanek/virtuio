...
export async function handleFormSubmission(
  ...
  if (!apiKey) {
    return {
      message:
        "Jejda, něco se pokazilo. Zkuste se nás kontaktovat telefonicky nebo pomocí našeho emailu.",
      success: false,
      errors: {},
    };
  }
  ...
  if (!validatedData.success) {
    ...
    return {
      message:
        "Nastala chyba při odesílání formuláře. Zkontrolujte, že máte vyplněná všechna povinná pole.",
      success: false,
      errors: errors,
    };
  }

  const { name, surname, email, phone, city, address, date, numberOfDays } =
    validatedData.data;
  ...
  if (leadError) {
    ...
    return {
      message:
        "Jejda, nastala chyba při odesílání formuláře. Zkuste nás kontaktovat telefonicky nebo pomocí našeho emailu.",
      success: false,
      errors: {},
    };
  }
  ...
  return {
    message:
      "Vaše poptávka byla úspěšně odeslána. Brzy Vás budeme kontaktovat.",
    success: true,
    errors: {},
  };
}
